...
### 1. 필수 라이브러리 설치
대부분의 맥/리눅스 환경에는 `ncurses`가 기본 설치되어 있으나, 없을 경우 아래 명령어로 설치합니다.
...
### ⚠️ 주의 사항
- **PDCurses 파일 제외:** 컴파일 시 윈도우용 라이브러리 파일인 `pdcurses.lib`나 `curses.h` 파일이 같은 폴더에 있어도 `-lncurses` 옵션이 시스템 라이브러리를 우선 참조하므로 무시해도 괜찮습니다.
